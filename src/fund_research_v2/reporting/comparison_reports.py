...
def render_comparison_report(path: Path, comparison: dict[str, object]) -> None:
    ...
    lines = [
        ...
        f"- previous_generated_at: {previous.get('generated_at', 'n/a')}",
        f"- previous_git_commit: {previous.get('git_commit', 'n/a')}",
        f"- current: {current.get('label', 'n/a')}",
        f"- current_generated_at: {current.get('generated_at', 'n/a')}",
        f"- current_git_commit: {current.get('git_commit', 'n/a')}",
        "",
        "## Freshness Notice",
        "",
        "- 本报告只比较 experiment_registry.jsonl 中最近两次完整实验记录。",
        "- 若后续又执行了 run-experiment，但尚未刷新 compare-experiments，则本报告会过期。",
        "- 当前建议把本报告与 experiment_registry.jsonl 的最后两条记录一起核对后再用于评审。",
        "",
        "## Summary",
        "",
    ]
