...
当前版本另外提供一个本地只读网站作为展示层：

- 只读取 `outputs/<data_source>/...` 下的既有产物
- 不参与数据抓取、因子计算、组合构建或回测
- 不改变现有 CSV / JSON / Markdown 输出契约
...
4. `run-ranking`
   对同月基金做横截面打分和排序
5. `run-backtest`
   使用“下月第 1 个交易日决策、T+2 到账、T+3 新组合开始收益”的研究执行代理做历史回测
6. `run-experiment`
...
- 使用 `decision_date` 驱动的研究执行代理
- `T` 日按旧组合净值赎回，旧组合包含 `T` 日收益
- `T+2` 日现金到账后申购
- `T+3` 日新组合开始承担收益
- 持有期收益基于 `fund_nav_pit_daily` 的日收益复利计算
- 按换手收取固定交易成本
...
- 调仓频率：月频
- 信号时点：最后一个完整自然月的月末
- 决策时点：下月第 1 个交易日 `decision_date`
- 执行代理：`T` 卖出、`T+2` 到账、`T+3` 新组合开始承担收益
- 组合方法：规则约束法
